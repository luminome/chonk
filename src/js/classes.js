...
class data_point_wudi {
		...
		get_mesh(){
			...
			if(this.data.m.length) {
				...
				// bars.userData.level = 2;//this.data.lv;//+1;
				// bars.userData.scans = true;
				// bars.userData.name = 'bars';
				...
				//
				//
				// const ost = UTIL.normalize_val(this.data.v[0], 0.01, this.data_spec.limits[0]);
				// const A2 = this.A.clone().setZ(ost);
				// const B2 = this.B.clone().setZ(ost);
				// const points = [this.A, A2, B2, this.B];
				//
				// const bar_geometry = new THREE.BufferGeometry().setFromPoints(points);
				// bar_geometry.setIndex([0, 1, 2, 2, 3, 0]);
				...
				//
				// const bar = new THREE.Mesh(bar_geometry, the_UPW_material);
				// bar.userData.level = this.data.lv;//+1;
				// const line = new THREE.Line(line_geometry, material);
				// line.userData.level = this.data.lv+1;
				//
				// //#const ost = UTIL.normalize_val(this.data.v[1], 0.01, this.data_spec.limits[1]);  //vars.map.s);
				//
				// line.translateZ(ost);//0.006);
...
class sector extends THREE.Group {
	...
	load_tile(data, zoom_level) {
		...
		// 	if(k.hasOwnProperty('static')){
		// 		k.levels[0][0] = 2;
		// 	}else{
		// 		if(k.levels.length > zoom_level-1){
		// 			k.current_zoom = zoom_level;
		// 			if(k.levels[zoom_level-1][k.frame] === 1) k.levels[zoom_level-1][k.frame] = 2;
		// 		}
		// 		else {
		// 			if (ek === 'wind' || ek === 'sst') {
		// 				let kzl = Math.min(zoom_level-1, k.levels.length-1);
		// 				if (k.levels[kzl][k.frame] === 1) k.levels[kzl][k.frame] = 2;
		// 			}
		// 		}
		// 	}
		// });

		// for(let k of this.data_keys){
		// 	console.log(k);
		//
		// 	// if(k.hasOwnProperty('static')){
		// 	// 	k.levels[0][0] = 2;
		// 	// }else if(k.levels[zoom_level-1][k.frame] === 1){
		// 	// 	k.levels[zoom_level-1][k.frame] = 2;
		// 	// }
		// }
		//console.log(zoom_level);

		// Object.keys(this.data_keys)
		// 	//filter((k) => this.data_keys[k]['lv'][zoom_level-1] === 1 || this.data_keys[k].hasOwnProperty('static'))
		// 	//.filter((k) => this.data_keys[k].levels[zoom_level-1])
		// 	.filter((k) => this.data_keys[k].levels[zoom_level-1][this.data_keys[k].frame] === 1 || this.data_keys[k].hasOwnProperty('static'))
		// 	.map((k) => {
		// 		this.data_keys[k].levels[zoom_level-1][this.data_keys[k].frame]  = 2;
		// 	});
